 ...
 */
package jp.co.senrido.excption.handler;
...
import jp.co.senrido.excption.AuthFailException;
import jp.co.senrido.excption.BusinessException;
import jp.co.senrido.excption.SystemException;
import jp.co.senrido.json.BaseResponse;
...
@ControllerAdvice
public class SenridoExceptionHandler {

	/** ログ */
	private static final Log log = LogFactory.getLog(SenridoExceptionHandler.class);
	...
	public BaseResponse handleBusinessException(BusinessException ex, WebRequest request) {
		...
		res.setResultCd(SenridoConstant.ResultCd.BUSSINESS_ERROR);
	...
	public BaseResponse handleSystemException(SystemException ex, WebRequest request) {
		...
		BaseResponse res = new BaseResponse();
		res.setResultCd(SenridoConstant.ResultCd.SYSTEM_ERROR);
		return res;
	}
	...
	private BaseResponse createBaseResponse(List<FieldError> list, Map<String, Object> model) throws Exception {
		...
		for (FieldError fieldError : list) {
			...
			info.setMessageType(name); // TODO サニタイズが必要でしょうか。
