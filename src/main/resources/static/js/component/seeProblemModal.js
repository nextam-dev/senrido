Vue.component('see-problem-modal', {
    template: (function () {/*
        <div class="modal-mask" id="seeProblemModal" v-show="displayFlg">
            <div class="modal-wrapper-basis">
	            <!--　モーダルヘッダー　 -->
	            <div class="modal-hedder row">
	                <div class="col-10">
	                    <label class="modal-hedder-title">見えないと困るもの</label>
	                </div>
	                <div class="col-2">
	            ...
	            <div class="modal-content">
	            	<div class="row">
						<div class="col-3 item-title">見えないと困るもの</div>
					</div>
	                <div class="row">
                        <div class="col-12 modal-item-value">
			                <label class="ef">
			                	<input type="text" v-model="criticalVisibilityNeedsInfo.whatToSee"/>
			                </label>
                        </div>
                    </div>
	            </div><!-- /modal-content -->
    ...
    */}).toString().match(/\/\*([^]*)\*\//)[1],
	...
	methods: {
		open: function (item) {
			...
			this.criticalVisibilityNeedsInfo.id = item.id;
			this.criticalVisibilityNeedsInfo.visitDate = item.visitDate;
    	},
    	update:function() {
    		var self = this;
    		self.showModalProcessing();
			var postItem = {
					criticalVisibilityNeedsInfo: self.criticalVisibilityNeedsInfo,
			};
			axios.post(editUrl('/s007MedicalRecord/upsertSeeProblem'), postItem)
			.then(response => {
